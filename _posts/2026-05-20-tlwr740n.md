...
Edit: I transfered the libs from the toolchain into linux and it didn't work, it caused a kernel panic on boot, I should have expected that XD

Also, here is the code that controls GPIO, it only blinks the first LED, I haven't really worked on figuring out every GPIO, I just copied this from Ghidra:
...
int FUN_004b8f24(void) { //open_device
  int sucess;

  DAT_005a2390 = open("/dev/ar7100_gpio_chrdev",2); // O_RDWR
  sucess = 0;
  if (DAT_005a2390 == -1) {
    puts("open device ar7100_gpio_chrdev failed");
    perror("open device ar7100_gpio_chrdev failed\n");
    sucess = 0xffffffff;
  }
  return sucess;
}

void FUN_004b8fa4(ulong param_1,int param_2) {//set_gpio
  int iVar1;
  int local_res4 [3];

  local_res4[0] = param_2;
  if ((0 < DAT_005a2390) || (iVar1 = FUN_004b8f24(), -1 < iVar1)) {
    ioctl(DAT_005a2390,param_1,local_res4);
  }
  return;
}

int main() {
   // printf() exibe a string entre aspas
   printf("Hello from GPIO");
   uint uVar2;
   uVar2 = 1;
   while (true) {
      uVar2 = uVar2 ^ 1;
      FUN_004b8fa4(0x8004b20a, uVar2);
      sleep(1);
}
   return 0;
}
